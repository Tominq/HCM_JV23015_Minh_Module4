...
JOIN Student s ON st.RN = s.RN
GROUP BY s.ten
HAVING AVG(st.Mark) = (SELECT MAX(AverageMark) FROM (SELECT AVG(Mark) AS AverageMark FROM StudentTest GROUP BY RN) AS subquery);
...
HAVING Student.ten LIKE 'T%' AND AVG(StudentTest.Mark) > 4.5;
